...
stores = pd.merge(stores, data['as'], how='left', on=['air_store_id']) #‘as’一共有829条餐厅数据，而这里一共821家餐厅,应该都是包含在'as'里面的
...
col = [c for c in train if c not in ['id', 'air_store_id', 'visit_date','visitors']] #除去列表中4项，一共50列的列名，后面模型训练要用到
...
date_info = pd.read_csv(PATH+'input/date_info.csv')
air_visit_data = pd.read_csv(PATH+'input/air_visit_data.csv')
